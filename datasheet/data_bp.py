from flask import (Blueprint, current_app, request, render_template, redirect,
                   url_for, abort)
from flask_table import (Table as HTMLTable, LinkCol as _LinkCol,
                         ButtonCol as _ButtonCol)
...
def _get_table(table_name):
    return get_table_obj(current_app.config['SQLALCHEMY_DATABASE_URI'],
                         table_name)
...
def bp_factory():
    ...
    def view(table):
        ...
        q = table_obj.select()
        out = list(run_sql(q, current_app.config['SQLALCHEMY_DATABASE_URI'],
                           as_text=False))
        ...
        html_table._cols['_edit'] = LinkCol(
            'Edit', '.edit', url_kwargs=pks_col_names,
            url_extra_kwargs=dict(table=table))
        html_table._cols['_delete'] = ButtonCol(
            'Delete', '.delete', url_kwargs=pks_col_names,
            url_extra_kwargs=dict(table=table))

        return render_template('data_view.html', t=html_table, table=table)
    ...
    def edit(table):
        ...
        if f.validate_on_submit():
            q = table_obj.update()\
                         .where(*_filter(table_obj, request.args))\
                         .values(**f.data)
            run_sql(q, current_app.config['SQLALCHEMY_DATABASE_URI'],
                    as_text=False)
            return redirect(url_for('.view', table=table))
